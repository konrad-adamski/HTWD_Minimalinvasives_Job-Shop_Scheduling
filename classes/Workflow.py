...
class JobOperationWorkflowCollection(UserDict):
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        :return: DataFrame with columns: Job, Routing_ID, Operation, Start, Duration, End
        """
        rows = []
    ...
    def from_dataframe(cls, df: pd.DataFrame,
                       ...
                       duration_column: str = "Duration", end_column: str = "End"):
        """
...
if __name__ == "__main__":
    ...
    for job_id, ops in restored.items():
        print(f"Job: {job_id}")
        for op in ops:
            print(f"  Seq {op.sequence_number}, Start {op.start_time}, Duration {op.duration}, End {op.end_time}")
