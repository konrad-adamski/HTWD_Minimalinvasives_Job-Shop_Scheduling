...
from omega.LocalCollection import JobCollection  # noch nicht getestet
from omega.Workflow import JobOperationWorkflow, JobOperationWorkflowCollection
...
class ProductionSimulation:
    def __init__(
            self, job_information_collection: Optional[JobCollection] = None,
        ...
        if job_information_collection is not None:
            ...
            self.job_information_collection = JobCollection()
        ...
        self.current_schedule = JobOperationWorkflowCollection()
        self.active_operations: Dict[Tuple[str, int], JobOperationWorkflow] = {}  # (job_id, operation) → operation
    ...
    def _job_process(self, job_id: str, job_operations: List[JobOperationWorkflow]):
    ...
    def _resume_operation_process(self, job_op: JobOperationWorkflow):
    ...
    def run(
        ...
        self.current_schedule = schedule_collection if schedule_collection else JobOperationWorkflowCollection() # oder self.current_schedule
        self.finished_operations_collection = JobOperationWorkflowCollection()
    ...
    def _log_job_resumed_on_machine(self, time_stamp, remaining_time, job_op: JobOperationWorkflow):
        if self.verbose:
    ...
    def _register_active_operation(self, job_op: JobOperationWorkflow, sim_start, sim_duration):
        updated_op = replace(
            job_op,
    ...
    def _add_finished_operation(self, job_op: JobOperationWorkflow, sim_start, sim_end):
        updated_op = replace(
            job_op,
