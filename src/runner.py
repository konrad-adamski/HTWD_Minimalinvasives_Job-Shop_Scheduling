...
def run_shifts(experiment_id: int,  shift_length: int, total_shift_number: int, logger: Logger):
    ...
    for shift_number in range(1, total_shift_number + 1):
        ...
        solver.print_model_info() # logge es stattdessen TODO

        # überprüfe ob ordner existiert, sonst lege an  TODO
        file_path = f"{logs_path}/exp{experiment_id:02d}/sh{shift_number:02d}.log"
        ...
        solver.print_solver_info() # logger stattdessen TODO
...
def run_shift(shift_number: int, shift_length: int):
    shift_start = shift_number * shift_length
    shift_end = (shift_number + 1) * shift_length
