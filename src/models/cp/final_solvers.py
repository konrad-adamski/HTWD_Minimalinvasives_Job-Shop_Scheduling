...
def solve_jssp_lateness_with_start_deviation(
        df_jssp: pd.DataFrame, df_times: pd.DataFrame, df_original_plan: pd.DataFrame | None,
        df_active: pd.DataFrame | None = None, job_column: str = "Job", earliest_start_column: str = "Arrival",
        w_t: int = 5, w_e: int = 1, w_first: int = 1, main_pct: float = 0.5, latest_start_buffer: int = 720,
        schedule_start: float = 1440.0, sort_ascending: bool | None = False, msg: bool = False,
        timeLimit: int = 3600, gapRel: float = 0.0) -> pd.DataFrame:
...
def solve_jssp_lateness_with_order_deviation(
        df_jssp: pd.DataFrame, df_times: pd.DataFrame, df_original_plan: pd.DataFrame | None,
        df_active: pd.DataFrame | None = None, job_column: str = "Job", earliest_start_column: str = "Arrival",
        w_t: int = 5, w_e: int = 1, w_first: int = 1, main_pct: float = 0.5, latest_start_buffer: int = 720,
        schedule_start: float = 1440.0, sort_ascending: bool | None = False, msg: bool = False,
    ...
    # 3. === Reihenfolgeabweichung vorbereiten ===
    original_machine_sequences = {}
    if df_original_plan is not None:
        for m, df_m in df_original_plan.groupby("Machine"):
            df_common = df_m[df_m[[job_column, "Operation"]]
                             .apply(tuple, axis=1)
                             .isin(df_jssp[[job_column, "Operation"]].apply(tuple, axis=1))]
            seq = df_common.sort_values("Start")[[job_column, "Operation"]].apply(tuple, axis=1).tolist()
            original_machine_sequences[m] = seq
    ...
    # 8. === Nebenbedingungen und Zielterme ===
    jobop_to_index = {(job, all_ops[j][o][0]): (j, o)
                      for j, job in enumerate(jobs)
    ...
    # 9. === Reihenfolgeabweichung modellieren ===
    for m, seq in original_machine_sequences.items():
        for i in range(len(seq)):
            for k in range(i + 1, len(seq)):
                a = seq[i]
                b = seq[k]
                if a in jobop_to_index and b in jobop_to_index:
                    ja, oa = jobop_to_index[a]
                    jb, ob = jobop_to_index[b]
                    a_start = starts[(ja, oa)]
                    b_start = starts[(jb, ob)]

                    violated = model.NewBoolVar(f"inv_{m}_{ja}_{oa}_vs_{jb}_{ob}")
    ...
    # 11. === Zielfunktion zusammensetzen ===
    weighted_part = model.NewIntVar(0, horizon * len(weighted_terms), "weighted_part")
    model.Add(weighted_part == sum(weighted_terms))

    deviation_part = model.NewIntVar(0, len(deviation_terms), "deviation_part")
    model.Add(deviation_part == sum(deviation_terms))

    first_op_delay = model.NewIntVar(0, horizon * len(jobs) * w_first, "first_op_delay")
    model.Add(first_op_delay == sum(first_delay_penalties))

    combined_lateness = model.NewIntVar(-10000000, 10000000, "combined_lateness")
    model.Add(combined_lateness == weighted_part + first_op_delay)
    ...
    print(f"Best Objective Bound  : {solver.BestObjectiveBound():.2f}")
    print(f"Laufzeit              : {solver.WallTime():.2f} Sekunden")
    print(f"Deviation-Paare       : {len(deviation_terms)}")
