...
class DataFrameEnrichment:
    ...
    # Add due dates ---------------------------------------------------------------------------------------------------
    @staticmethod
    def add_groupwise_lognormal_due_dates_by_group_mean(
            df_times_temp: pd.DataFrame, sigma: float = 0.2, routing_column: str = "Routing_ID",
            earliest_start_column="Ready Time", end_column: str = "End", due_date_column: str = "Due Date",
            seed: Optional[int] = 42) -> pd.DataFrame:
        """
        Generate stochastic due_dates per routing group using log-normal-distributed flow budgets.

        For each group in the specified routing column, a log-normal distribution is fitted such that
        its mean matches the group's average flow time (End - Ready Time). Each due_date is then sampled
        ...
        :param routing_column: Column used to group jobs for separate due_date distributions.
        :param seed: Random seed for reproducibility.
        :return: Copy of the input DataFrame with an additional column 'due_date'.
        """
        if seed is not None:
            np.random.seed(seed)

        df_times = df_times_temp.copy()
        df_times[due_date_column] = np.nan
        ...
        for routing_id, grp in df_times.groupby(routing_column):
            ...
            # Für jede Zeile in Gruppe eine due_date aus LogNormal(mu, sigma)
            flow_budgets = np.random.lognormal(mean=mu, sigma=sigma, size=len(grp))
            df_times.loc[grp.index, due_date_column] = df_times.loc[grp.index, earliest_start_column] + np.round(
                flow_budgets)

        return df_times

    @staticmethod
    def ensure_reasonable_due_dates(
            df_times: pd.DataFrame, min_coverage: float = 0.90,
            earliest_start_column="Ready Time", end_column: str = "End",
            due_date_column: str = "Due Date", total_duration_column="Total Processing Time") -> pd.DataFrame:
        """
        Ensures that each job's due_date covers at least a minimum percentage of its total processing time.
        Also removes the 'End' column if present.

        :param df_times: DataFrame with at least the columns ['Ready Time', 'due_date', 'Job Processing Time'].
        :param min_coverage: Minimum fraction (0–1) of the job processing time that must be covered by the due_date.
                             Defaults to 0.90 (i.e. 90% of total processing time).
        :return: DataFrame with updated 'due_date' values and 'End' column removed if present.
        """
        min_coverage = min(min_coverage, 1.0)

        df_times[due_date_column] = np.maximum(
            df_times[due_date_column],
            df_times[earliest_start_column] + df_times[total_duration_column] * min_coverage
        )
        df_times[due_date_column] = np.ceil(df_times[due_date_column]).astype(int)
