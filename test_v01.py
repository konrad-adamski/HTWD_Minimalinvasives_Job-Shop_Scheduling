...
class JobOperationProblemCollection:
    ...
    """
    def __init__(self, routing_collectioninitions: RoutingOperationCollection):
        self.routing_collectioninitions = routing_collectioninitions
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        for job_op in self.job_operations:
            routing_op = self.routing_collectioninitions.get_operation(job_op.routing_id, job_op.sequence_number)
            if routing_op:
                rows.append({
                    "Job": job_op.job_id,
    ...
    def get_full_operation(self, job_id: str, routing_id: str, sequence_number: int) -> Optional[tuple]:
        routing_op = self.routing_collectioninitions.get_operation(routing_id, sequence_number)
        if routing_op:
...
if __name__ == "__main__":
    ...
    routing_collection = RoutingOperationCollection()
    routing_collection.add_operation("R25", 0, "M1", 5)
    routing_collection.add_operation("R25", 1, "M2", 3)
    routing_collection.add_operation("R25", 2, "M3", 4)
    routing_collection.add_operation("R30", 0, "M1", 2)
    routing_collection.add_operation("R30", 1, "M3", 6)
    routing_collection.sort_operations()

    # 2. JobOperationProblemCollection mit Jobs und Routings
    job_problem = JobOperationProblemCollection(routing_collection)
